...
class WirelessEnvironmentSACPA(WirelessEnvironmentBase):
        ...
        return gym.spaces.Box(
            low=np.array(
                [
                    np.zeros(
                        shape=(num_devices)
                    ),  # Quality of Service Satisfaction of each device on Sub6GHz
                    np.zeros(
                        shape=(num_devices)
                    ),  # Quality of Service Satisfaction of each device on mmWave,
                    np.zeros(
                        shape=(num_devices)
                    ),  # Number of received packets of each device on Sub6GHz of previous time step
                    np.zeros(
                        shape=(num_devices)
                    ),  # Number of received packets of each device on mmWave of previous time step
                    np.zeros(
                        shape=(num_devices)
                    ),  # Average Rate of each device on Sub6GHz of previous time step
                    np.zeros(
                        shape=(num_devices)
                    ),  # Average Rate of each device on mmWave of previous time step
                    np.zeros(
                        shape=(num_devices)
                    ),  # Power of each device on Sub6GHz on previous time step
                    np.zeros(
                        shape=(num_devices)
            ...
            high=np.array(
                [
                    ...
                    np.full(
                        shape=(num_devices), fill_value=L_max
                    ),  # Number of received packets of each device on Sub6GHz of previous time step
                    np.full(
                        shape=(num_devices), fill_value=L_max
                    ),  # Number of received packets of each device on mmWave of previous time step,
        ...
        return gym.spaces.Box(
            low=np.array(
                [
                    np.zeros(
                        shape=(num_devices)
                    ),  # Number of packets to send of each device on Sub6GHz,
                    np.zeros(
                        shape=(num_devices)
                    ),  # Number of packets to send of each device on mmWave,
    ...
    def _compute_rewards(self, agent: str) -> Dict[str, float]:
        ...
        instance_reward = (
            self.reward_coef["reward_qos"] * reward_qos
            + self.reward_coef["reward_power"] * reward_power
        )
